...
### 6. 细节优化 (Polishing)
*   **Fix**: 首页宫格分类改为直接展示前 12 个民族，移除了“全部商品”入口，利用率更高且更美观。
*   **Fix**: 收藏页价格显示错误（便宜100倍），因组件单位为“分”而数据为“元”，已修正 `item.price * 100`。

### 7. 新增管理模块 (New Modules)
*   **Feature**: 新增“发票管理”页面 (`pages/usercenter/invoice`)，支持从订单确认页自动生成记录。
*   **Feature**: 新增“退换货管理”页面 (`pages/usercenter/after-sale`)，支持从订单列表页发起申请。
*   **Logic**: 实现了“发票”和“退换货”的 Storage 动态闭环，不再是纯静态页面。
*   **Logic**: 申请退款后自动从订单列表中移除，防止重复操作。

## 📂 关键文件变更 (File Changes)
### 1. `services/home/home.js`
> 首页数据获取服务
- **Change**: 引入 `ethnicGoodsList`，动态生成前 12 个民族作为分类导航。
